...
describe('processPreAttackSpecials', () => {
  ...
  describe('clearAllBlock', () => {
    it('양측 방어력 0으로 설정', () => {
      ...
      expect(result.attacker.def).toBe(false);
      expect(result.defender.def).toBe(false);
    });

    it('방어력이 있을 때만 이벤트 발생', () => {
      const result = processPreAttackSpecials({
        card: { special: 'clearAllBlock', damage: 40, name: '필사의 일격' },
        attacker: { hp: 100, block: 15 },
        defender: { hp: 100, block: 25 },
        attackerName: 'player'
      });
    ...
    it('양측 방어력이 0이면 이벤트 없음', () => {
      const result = processPreAttackSpecials({
        card: { special: 'clearAllBlock', damage: 40, name: '필사의 일격' },
        attacker: { hp: 100, block: 0 },
        defender: { hp: 100, block: 0 },
        attackerName: 'player'
      });
  ...
  describe('doubleDamageIfSolo', () => {
    it('유일한 공격카드면 2배 피해', () => {
      const result = processPreAttackSpecials({
        ...
        defender: { hp: 100 },
        attackerName: 'player',
        battleContext: { playerAttackCards: [{ id: 'kick' }] }
      });
    ...
    it('여러 공격카드면 그대로', () => {
      const result = processPreAttackSpecials({
        ...
        defender: { hp: 100 },
        attackerName: 'player',
        battleContext: { playerAttackCards: [{ id: 'kick' }, { id: 'stab' }] }
      });
  ...
  describe('agilityBonus', () => {
    ...
    it('민첩 3이면 +15', () => {
      const result = processPreAttackSpecials({
        card: { special: 'agilityBonus', damage: 20, name: '취권' },
        attacker: { hp: 100, agility: 3 },
        defender: { hp: 100 },
        attackerName: 'player'
      });
...
describe('processPostAttackSpecials', () => {
  ...
  describe('vulnIfNoBlock', () => {
    it('방어력 없으면 취약 부여', () => {
      const result = processPostAttackSpecials({
        card: { special: 'vulnIfNoBlock', damage: 14, name: '검 빙글빙글' },
        attacker: { hp: 100 },
        defender: { hp: 100, def: false, block: 0, tokens: {} },
        attackerName: 'player',
        damageDealt: 14
      });
    ...
    it('방어력 있으면 취약 부여 안함', () => {
      const result = processPostAttackSpecials({
        card: { special: 'vulnIfNoBlock', damage: 14, name: '검 빙글빙글' },
        attacker: { hp: 100 },
        defender: { hp: 100, def: true, block: 10, tokens: {} },
        attackerName: 'player',
        damageDealt: 14
      });
      expect(result.events.length).toBe(0);
  ...
  describe('doubleVulnIfNoBlock', () => {
    it('방어력 없으면 2스택 취약 부여', () => {
      const result = processPostAttackSpecials({
        card: { special: 'doubleVulnIfNoBlock', damage: 10, name: '미늘작살' },
        attacker: { hp: 100 },
        defender: { hp: 100, def: false, block: 0, tokens: {} },
        attackerName: 'player',
        damageDealt: 10
  ...
  describe('hitOnEnemyAction', () => {
    it('persistent_strike 토큰 부여', () => {
      const result = processPostAttackSpecials({
        card: { special: 'hitOnEnemyAction', damage: 20, name: '질긴 놈' },
        attacker: { hp: 100, tokens: {} },
        defender: { hp: 100 },
        attackerName: 'player',
        damageDealt: 20
      });
  ...
  describe('halfEnemyEther', () => {
    it('half_ether 토큰 부여', () => {
      const result = processPostAttackSpecials({
        card: { special: 'halfEnemyEther', damage: 30, name: '에테르 커터' },
        attacker: { hp: 100 },
        defender: { hp: 100, tokens: {} },
        attackerName: 'player',
        damageDealt: 30
      });
...
describe('processQueueCollisions', () => {
  ...
  it('여러 충돌 처리', () => {
    const queue = [
      { actor: 'player', card: { name: '박치기1', special: 'destroyOnCollision' }, sp: 5 },
      { actor: 'player', card: { name: '박치기2', special: 'destroyOnCollision' }, sp: 10 },
      { actor: 'enemy', card: { name: '적1' }, sp: 5 },
      { actor: 'enemy', card: { name: '적2' }, sp: 10 },
      { actor: 'enemy', card: { name: '적3' }, sp: 15 }
    ];
    const result = processQueueCollisions(queue, () => {});
  ...
  it('같은 sp에 여러 적 카드면 모두 파괴', () => {
    const queue = [
      { actor: 'player', card: { name: '박치기', special: 'destroyOnCollision' }, sp: 7 },
      { actor: 'enemy', card: { name: '적1' }, sp: 7 },
...
describe('processCollisionSpecials', () => {
  it('destroyOnCollision이면 destroyed true', () => {
    const result = processCollisionSpecials({
      card: { name: '박치기', special: 'destroyOnCollision' },
      enemyCard: { name: '적 카드' },
      attackerName: 'player'
    });
