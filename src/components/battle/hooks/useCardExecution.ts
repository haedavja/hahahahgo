...
import type { Card, HandAction, BreachSelection, TokenEntity, ParryReadyState, TokenState, EnemyUnitState, RecallSelection } from '../../../types';
import type { BattleEvent, SingleHitResult, CardPlaySpecialsResult, PostCombatOptions } from '../../../types/combat';
import type { BattleActions } from './useBattleState';
import type { NextTurnEffects } from '../reducer/battleReducerState';
...
export interface CardExecutionDeps {
  ...
  battleRef: MutableRefObject<{
    queue: HandAction[];
    qIndex: number;
    player: TokenEntity & { hp: number; block?: number; def?: boolean };
    enemy: TokenEntity & { hp: number; block?: number; def?: boolean };
    nextTurnEffects?: Partial<NextTurnEffects>;
    hand?: Card[];
    deck?: Card[];
    discardPile?: Card[];
    vanishedCards?: Card[];
  ...
  creationQueueRef: MutableRefObject<Card[]>;
  ...
  relics: string[];
  cardUpgrades: Record<string, number | string>;
  turnEtherAccumulated: number;
  enemyTurnEtherAccumulated: number;
  playerTimeline: HandAction[];
  resolvedPlayerCards: number;
  nextTurnEffects: Partial<NextTurnEffects>;
  pathosTurnEffects?: Partial<NextTurnEffects>;
  pathosNextCardEffects?: {
    guaranteeCrit?: boolean;
    setSpeed?: number;
    aoe?: boolean;
  };
  safeInitialPlayer: TokenEntity & { hp: number; block?: number };

  // Actions
  actions: BattleActions & {
    setPlayer: (p: TokenEntity & { hp: number; block?: number; def?: boolean }) => void;
    setEnemy: (e: TokenEntity & { hp: number; block?: number; def?: boolean; units?: EnemyUnitState[] }) => void;
    setQueue: (q: HandAction[]) => void;
    setQIndex: (i: number) => void;
    setPhase: (p: string) => void;
    setPostCombatOptions: (o: PostCombatOptions | null) => void;
    setActionEvents: (e: Record<number, BattleEvent[]>) => void;
    setNextTurnEffects: (e: Partial<NextTurnEffects>) => void;
    setCardUsageCount: (c: Record<string, number>) => void;
    setDeck: (d: Card[]) => void;
    setDiscardPile: (d: Card[]) => void;
    setHand: (h: Card[]) => void;
    setEnemyUnits: (u: EnemyUnitState[]) => void;
