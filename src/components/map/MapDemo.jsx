...
const PATCH_VERSION_TAG = "11-16-13:07"; // 다음 패치마다 여기를 최신 시간(월-일-시:분, KST)으로 갱신하세요.

/* v11-16-13:07 갱신 내역
 * - 전투 로그 색상 개선: 플레이어 행동 파란색(#60a5fa), 몬스터 행동 옅은 붉은색(#fca5a5)
 * - 진행 단계 로그 최적화: 예상 피해량 로그 제거, 실제 전투 로그만 표시
 * - 적 성향 힌트 UI: 몬스터 이름 위에 힌트 표시 (💡 아이콘), 로그에서 제거
 * - 손패 카드 중앙 정렬: 대응/진행 단계에서 카드가 중앙에 배치
 * - 손패 인터페이스 축소: 높이 축소로 화면 공간 효율 향상
 * - 타임라인 헤더 제거: 상단 텍스트 완전 삭제로 깔끔한 UI
 * - 전투 로그 자동 스크롤: 로그 갱신 시 자동으로 맨 아래로 이동
 */
