...
function selectPlayerActions(player: SimPlayerState): { cards: (Card | AICard)[]; indices: number[] } {
  ...
  const calculateComboBonus = (selectedCards: (Card | AICard)[], newCard: Card | AICard): number => {
    const testCards = [...selectedCards, newCard].map(c => ({
      id: c.id!,
...
function selectEnemyActions(enemy: SimEnemyState, turnNumber: number): AICard[] {
  // 패턴이 있으면 패턴 사용
  const patternConfig = ENEMY_PATTERNS[enemy.id as keyof typeof ENEMY_PATTERNS];
  let mode: AIMode | null = null;

  if (patternConfig) {
    const action = getPatternAction(enemy.id, turnNumber, enemy.hp, enemy.maxHp);
    if (action) {
      const actionMode = patternActionToMode(action, patternConfig as Parameters<typeof patternActionToMode>[1]);
...
function simulateTurn(
  ...
  if (playerSelection.cards.length > 0) {
    const comboCards = playerSelection.cards.map(c => ({
      id: c.id!,
      actionCost: c.actionCost || 1,
      type: c.type || 'attack',
      traits: (c as Card).traits || [],
  ...
  for (let stepIndex = 0; stepIndex < timeline.length; stepIndex++) {
    ...
    try {
      ...
      if (result.updatedState) {
        combatState.player = result.updatedState.player as Combatant;
        combatState.enemy = result.updatedState.enemy as Combatant;
      }
      ...
      if (isAttack && hasKnockbackTrait(step.card) && finalDealt > 0) {
        const knockbackAmount = ((step.card as Card).knockbackAmount as number) || 3;
  ...
  player.tokens = combatState.player.tokens ?? player.tokens;
  player.strength = combatState.player.strength || player.strength;
  enemy.hp = combatState.enemy.hp;
  enemy.tokens = combatState.enemy.tokens ?? enemy.tokens;
...
export function runSimulation(config: SimulationConfig): SimulationStats {
  ...
  const avgPlayerDamageDealt = results.reduce((sum, r) => sum + r.playerDamageDealt, 0) / totalBattles;
  const avgEnemyDamageDealt = results.reduce((sum, r) => sum + r.enemyDamageDealt, 0) / totalBattles;
  ...
  return {
    ...
    avgTurns,
    avgPlayerDamageDealt: avgPlayerDamageDealt,
    avgEnemyDamageDealt: avgEnemyDamageDealt,
    avgPlayerFinalHp,
    cardEfficiency: {},
