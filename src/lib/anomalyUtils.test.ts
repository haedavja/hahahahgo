...
import {
  ...
  createAnomalyEnemy,
  createAnomalyEffect,
  type TestActiveAnomaly,
  type TestForcedAnomaly,
  type TestAnomalyPlayer,
} from '../test/factories';
...
describe('anomalyUtils', () => {
  ...
  describe('selectBattleAnomalies', () => {
    it('mapRisk가 50 미만이고 발동 실패하면 빈 배열을 반환해야 함', () => {
      ...
      const result = selectBattleAnomalies(20, false, undefined);
      expect(result).toEqual([]);
    });

    it('mapRisk가 50 이상이면 항상 이변이 발동해야 함', () => {
      const result = selectBattleAnomalies(50, false, undefined);
      expect(result.length).toBeGreaterThan(0);
    });

    it('일반 전투는 1개의 이변만 반환해야 함', () => {
      const result = selectBattleAnomalies(60, false, undefined);
      expect(result).toHaveLength(1);
    });

    it('개발자 모드 강제 이변이 적용되어야 함', () => {
      const devForced: TestForcedAnomaly[] = [
        createForcedAnomaly('ether_void', 3)
      ];
    ...
    it('존재하지 않는 이변 ID는 필터링되어야 함', () => {
      const devForced: TestForcedAnomaly[] = [
        createForcedAnomaly('nonexistent_anomaly', 1)
      ];
      const result = selectBattleAnomalies(10, false, devForced);
      expect(result).toEqual([]);
    });
  ...
  describe('applyAnomalyEffects', () => {
    it('빈 이변 배열은 원본 플레이어를 반환해야 함', () => {
      const player = createAnomalyPlayer();
      const result = applyAnomalyEffects([], player, createAnomalyEnemy());
    ...
    it('null 이변 배열은 원본 플레이어를 반환해야 함', () => {
      const player = createAnomalyPlayer();
      const result = applyAnomalyEffects(null as unknown as TestActiveAnomaly[], player, createAnomalyEnemy());
    ...
    it('ETHER_BAN 효과가 적용되어야 함', () => {
      const player = createAnomalyPlayer();
      const anomalies: TestActiveAnomaly[] = [
        createActiveAnomaly(1, {
          getEffect: () => createAnomalyEffect('ETHER_BAN', { description: 'No ether' })
        })
      ];

      const result = applyAnomalyEffects(anomalies, player, createAnomalyEnemy());
    ...
    it('ENERGY_REDUCTION 효과가 적용되어야 함', () => {
      ...
      const anomalies: TestActiveAnomaly[] = [
        createActiveAnomaly(1, {
          getEffect: () => createAnomalyEffect('ENERGY_REDUCTION', { value: 2, description: '-2 energy' })
        })
      ];
    ...
    it('SPEED_REDUCTION 효과가 적용되어야 함', () => {
      ...
      const anomalies: TestActiveAnomaly[] = [
        createActiveAnomaly(1, {
          getEffect: () => createAnomalyEffect('SPEED_REDUCTION', { value: 3, description: '-3 speed' })
        })
      ];
    ...
    it('DRAW_REDUCTION 효과가 적용되어야 함', () => {
      ...
      const anomalies: TestActiveAnomaly[] = [
        createActiveAnomaly(1, {
          getEffect: () => createAnomalyEffect('DRAW_REDUCTION', { value: 0.1, description: '-10% draw' })
        })
      ];
    ...
    it('INSIGHT_REDUCTION 효과가 적용되어야 함', () => {
      ...
      const anomalies: TestActiveAnomaly[] = [
        createActiveAnomaly(1, {
          getEffect: () => createAnomalyEffect('INSIGHT_REDUCTION', { value: 1, description: '-1 insight' })
        })
      ];
    ...
    it('여러 이변 효과가 누적되어야 함', () => {
      ...
      const anomalies: TestActiveAnomaly[] = [
        createActiveAnomaly(1, {
          name: 'Anomaly 1',
          getEffect: () => createAnomalyEffect('ENERGY_REDUCTION', { value: 1, description: '-1 energy' })
        }),
        createActiveAnomaly(2, {
          name: 'Anomaly 2',
          emoji: '💫',
          getEffect: () => createAnomalyEffect('ENERGY_REDUCTION', { value: 2, description: '-2 energy' })
        })
    ...
    it('이변을 UI 표시용 포맷으로 변환해야 함', () => {
      const anomalies: TestActiveAnomaly[] = [
        createActiveAnomaly(2, {
          ...
          getEffect: (level: number) => createAnomalyEffect('ETHER_BAN', { description: `Level ${level} effect` })
        })
      ];
  ...
  describe('applyAnomalyPenalties', () => {
    ...
    it('패널티가 적용되어야 함', () => {
      const player: TestAnomalyPlayer = createAnomalyPlayer({
        energyPenalty: 2,
        speedPenalty: 10,
        insightPenalty: 1
      });
  ...
  describe('applyDrawPenalty', () => {
    ...
    it('패널티가 적용되어야 함', () => {
      const player = createAnomalyPlayer({ drawPenalty: 0.2 });
      expect(applyDrawPenalty(0.8, player)).toBeCloseTo(0.6); // 0.8 - 0.2
    });

    it('확률은 0 이하로 내려가지 않아야 함', () => {
      const player = createAnomalyPlayer({ drawPenalty: 1 });
      expect(applyDrawPenalty(0.5, player)).toBe(0);
    });

    it('확률은 1을 초과하지 않아야 함', () => {
      const player = createAnomalyPlayer({ drawPenalty: -0.5 }); // 음수 패널티 = 보너스
