...
describe('dungeonChoices', () => {
  describe('canSelectChoice', () => {
    ...
    it('완료된 선택지는 선택 불가능해야 함', () => {
      const choice = { text: 'Test choice' };
      const result = canSelectChoice(choice, defaultStats, { completed: true });
    ...
    it('반복 불가능한 선택지는 한 번만 선택 가능해야 함', () => {
      ...
      const result = canSelectChoice(choice, defaultStats, { attempts: 1 });
    ...
    it('반복 가능한 선택지는 여러 번 선택 가능해야 함', () => {
      ...
      const result = canSelectChoice(choice, defaultStats, { attempts: 3 });
    ...
    it('요구 아이템이 없으면 선택 불가능해야 함', () => {
      const choice = { text: 'Test choice', requirements: { item: 'golden_key' } };
      const result = canSelectChoice(choice, defaultStats, {}, { items: [] });
    ...
    it('요구 아이템이 있으면 선택 가능해야 함', () => {
      const choice = { text: 'Test choice', requirements: { item: 'golden_key' } };
      const result = canSelectChoice(choice, defaultStats, {}, { items: ['golden_key'] });

      expect(result.canSelect).toBe(true);
    });

    it('요구 스탯 미달이면 선택 불가능하고 숨겨져야 함', () => {
      const choice = { text: 'Test choice', requirements: { strength: 10 } };
      const result = canSelectChoice(choice, defaultStats);
    ...
    it('스케일링 요구 스탯 미달이면 선택 불가능해야 함', () => {
      ...
      const result = canSelectChoice(choice, defaultStats, { attempts: 2 });
    ...
    it('스케일링 요구 스탯 충족이면 선택 가능해야 함', () => {
      ...
      const result = canSelectChoice(choice, { strength: 5 }, { attempts: 1 });
  ...
  describe('executeChoice', () => {
    it('특수 선택지 사용 시 즉시 완료되어야 함', () => {
      ...
      const result = executeChoice(choice, {}, {}, specialOverride);
    ...
    it('경고 시점에 경고 메시지가 포함되어야 함', () => {
      ...
      const result = executeChoice(choice, {}, { attempts: 2 });
    ...
    it('진행 텍스트가 시도 횟수에 따라 표시되어야 함', () => {
      ...
      const result1 = executeChoice(choice, {}, { attempts: 0 });
      expect(result1.message).toBe('First');

      // attempts 1 → 실행 후 attempts=2 → progressText[1] = 'Second'
      const result2 = executeChoice(choice, {}, { attempts: 1 });
    ...
    it('최대 시도 횟수에 도달하면 완료되어야 함', () => {
      const choice = {
        ...
        maxAttempts: 2,
        outcomes: {
          success: { type: 'success', text: 'Done!' }
        }
      };

      const result = executeChoice(choice, {}, { attempts: 1 });
  ...
  describe('getOverpushPenalty', () => {
    ...
    it('과잉 선택 시 확률에 따라 패널티를 반환해야 함', () => {
      const choice = {
        text: 'Test choice',
        warningAtAttempt: 2,
        outcomes: { failure: { type: 'failure', text: 'Failed!' } }
      };
  ...
  describe('getChoiceDisplayInfo', () => {
    ...
    it('숨겨진 선택지는 ???로 표시되어야 함', () => {
      const choice = { text: 'Secret choice', requirements: { strength: 10 } };
      const result = getChoiceDisplayInfo(choice, defaultStats);
    ...
    it('특수 선택지가 있으면 대체 텍스트를 표시해야 함', () => {
      ...
      const result = getChoiceDisplayInfo(choice, defaultStats, {}, specialOverride);
    ...
    it('반복 선택지는 시도 횟수를 표시해야 함', () => {
      ...
      const result = getChoiceDisplayInfo(choice, defaultStats, { attempts: 2 });
    ...
    it('선택 불가능한 이유를 서브텍스트로 표시해야 함', () => {
      const choice = { text: 'Test choice', repeatable: true, maxAttempts: 3 };
      const result = getChoiceDisplayInfo(choice, defaultStats, { attempts: 3 });
