...
인수인계 대상: GPT Codex
## �߰� �޸�: ����(Insight) ���� ���� �غ�
- ���� ������ ���� �̱����̸� �⺻�� 0���� �����ϵ��� ���� �ʵ�(playerInsight) �߰� �ʿ�.
- ���� ���̷ε�/�ʱ� ���¿� ���� ����: LegacyBattleScreen �� buildBattlePayload �� LegacyBattleApp initialPlayerState/�ʱ� setPlayer���� ���� ����.
- DevTools�� ���� �Է�/���� UI �߰� �ʿ�(Strength/Agility�� ������ ���� �Է�).
- �帷(���Ͱ� ������ �����ϴ� ����)�� ���� ������. ���� ���� �����Ϳ� �帷 �ʵ带 �ΰ� ���� ��� �� ���� ���� �߰� ����.

## 추가 메모 (적 에테르 대칭 작업 미완)
- 적 에테르 텍스트 패널을 플레이어와 동일한 절대 배치로 옮기는 중. 현재 적 HP 블록 안의 +PT/=PT 블록은 제거했지만, 에테르바 옆 패널 배치/중복 정리가 미완성 상태.
- 몬스터 배율(×) 표시는 enemyCombo 한 번만 노출하도록 해야 함. 지금은 표시 위치 재조정 필요.
- 워킹 변경 파일: src/components/battle/LegacyBattleApp.jsx (미커밋). 나머지 파일은 커밋 반영.

## 이번 세션 요약 (추가)
- 통찰 연출: Lv3를 모든 단계(select/respond/resolve)에서 동작하도록 확장. 통찰 툴팁은 화면 고정으로 표시하도록 조정.
- 에테르 시스템: 인플레이션 10%로 완화. 큰 수(K/M/B) 축약 표기 + hover 툴팁(전체값) + 진행률 툴팁 추가. 에테르 바는 10칸 색상을 순환하지만 실제 슬롯은 계속 증가하도록 변경.
- 에테르 승리 흐름: 몬스터 기본 소지량 300pt 부여. 턴 종료 시 플레이어/몬스터 에테르를 동일 규칙으로 누적·범람·디플레이션 후 적용하고, 적용치 차이를 서로 이동시키도록 변경(플레이어도 에테르를 잃을 수 있음).
- UI 대칭 작업(미완): 적 에테르 텍스트를 플레이어와 동일한 구조로 절대 배치하려다 중간 상태. HP 블록 안의 +/= 표시는 제거했지만, 에테르바 옆에 단일 패널을 배치하는 작업은 미완료. 곱배율 중복 제거 및 위치 조정 필요. 워킹 변경 파일: src/components/battle/LegacyBattleApp.jsx (미커밋 변경 포함).
- 최근 커밋: 1a3c813(통찰0시 적 콤보 숨김/에테르바 진행률 수정), 70e87f6(인플레이션 10%/큰 수 UI), 4854d0b(Lv3 연출 전 단계), ea00d18(툴팁 글로벌), a13cce0(통찰 아이콘 위치), d422387(적 에테르 흐름 대칭), 5dbb0b7(타임라인 숨김 조건) 등.
