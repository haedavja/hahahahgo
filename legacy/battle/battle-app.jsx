          ...
          return (
            <div className="bg-slate-900 rounded p-3 text-slate-200 text-sm mb-2">
              <div className="flex flex-wrap gap-4">
                <div>🗡️ 플레이어 예상 가한 피해: <span className="text-emerald-300 font-bold">{res.pDealt}</span></div>
                <div>💥 플레이어 예상 받은 피해: <span className="text-rose-300 font-bold">{res.pTaken}</span></div>
              </div>
              <div className="mt-1 flex flex-wrap gap-4">
                <div>플레이어 HP <span className="text-cyan-400">{player.hp}</span> → <span className="font-bold">{res.finalPHp}</span></div>
                <div>몬스터 HP <span className="text-orange-400">{enemy.hp}</span> → <span className="font-bold">{res.finalEHp}</span></div>
                {willOverdrive && <div className="text-cyan-300">(폭주 적용 미리보기)</div>}
              </div>
            </div>
          );
        }
        ...
        function EtherBar({ pts, color="cyan", label }){
          ...
          return (
            <div className="flex flex-col items-center w-12">
              ...
              <div className={`text-xs mt-1 ${palette.text}`}>누적 {pts}pt (슬롯 x{slots})</div>
            </div>
          );
        }
        ...
        function Game(){
          ...
          const initialEtherRef = useRef(0);
          const resultSentRef = useRef(false);
          const sendHostMessage = useCallback((message)=>{
          ...
          const notifyBattleResult = useCallback((resultType)=>{
            if(!resultType || resultSentRef.current) return false;
            const delta = player.etherPts - initialEtherRef.current;
            ...
            return success;
          }, [player.etherPts, sendHostMessage]);
          ...
          useEffect(()=>{
            const handler = (event)=>{
              ...
              if(data.type === 'battleInit'){
                const payload = data.payload || {};
                if(payload.player){
                  setPlayer(prev=>{
                    const incoming = payload.player;
                    const nextHp = incoming.hp ?? prev.hp;
                    const nextMaxHp = incoming.maxHp ?? incoming.hp ?? prev.maxHp;
                    const nextEnergy = incoming.energy ?? prev.energy;
                    const nextEther = typeof incoming.etherPts === 'number' ? incoming.etherPts : prev.etherPts;
                ...
                if(payload.enemy){
                  setEnemy(prev=>{
                    const incoming = payload.enemy;
                    const baseDeck = incoming.deck || prev?.deck || ENEMIES[enemyIndex]?.deck || [];
                    const hp = incoming.hp ?? incoming.maxHp ?? prev?.maxHp ?? 30;
                    return {
                      ...(prev||{}),
                      deck: baseDeck,
                      name: incoming.name ?? prev?.name ?? '적',
                      hp,
                      maxHp: incoming.maxHp ?? hp,
                      vulnMult:1,
                      vulnTurns:0,
                      block:0,
                      counter:0,
                ...
                setPhase('select');
                addLog('🌐 외부 전투 데이터 수신 완료');
              }
            };
            window.addEventListener('message', handler);
            sendHostMessage({ type:'battleReady' });
            return ()=> window.removeEventListener('message', handler);
          }, [addLog, sendHostMessage, enemyIndex]);
          ...
          return (
            <div className="w-full min-h-screen bg-gradient-to-br from-slate-950 via-purple-950 to-slate-950 pb-64">
              ...
              <div className="max-w-[1600px] mx-auto p-4">
                ...
                <div className="panel-enhanced p-4 mb-6">
                  <div className="text-white font-bold mb-3 flex items-center gap-2">
                    ...
                    타임라인 (누적 속도) — {phase==='select'? '선택' : (phase==='respond'? '대응/예측' : (phase==='resolve' ? '진행' : '결과'))} <span className="text-xs text-slate-300 ml-2">(동률 시 플레이어 우선)</span>
                  </div>
                ...
                {/* Battle Arena - 타임라인과 같은 너비 */}
                <div className="battle-arena p-8 mb-6">
                      <div className="flex justify-between items-center">
                        {/* Player Ether Bar */}
                        <EtherBar pts={player.etherPts} label="에테르" />

                        {/* Player Side */}
                        <div className="flex items-center gap-6">
                          <div className="character-display">🧙‍♂️</div>
                          <div>
                            <div className="text-cyan-400 font-black text-2xl mb-2">플레이어</div>
                            ...
                            <div className="flex gap-2 flex-wrap">
                              {player.block>0 && <span className="px-3 py-1 rounded-full bg-blue-700 text-white text-sm font-bold">🛡️ {player.block}</span>}
                              {player.vulnMult>1 && <span className="px-3 py-1 rounded-full bg-red-700 text-white text-sm font-bold">취약 ×{player.vulnMult.toFixed(1)}</span>}
                              {player.etherOverdriveActive && <span className="px-3 py-1 rounded-full bg-cyan-600 text-white text-sm font-bold animate-pulse">⚡폭주</span>}
                            </div>
                        ...
                        <div className="text-center">
                          {/* Phase Indicator - 대응/진행 단계 표시 */}
                          {phase==='respond' && (
                            <div className="bg-slate-800 rounded-lg px-6 py-3 mb-4 mx-auto" style={{width: '400px', maxWidth: '90vw'}}>
                              <div className="text-white font-bold text-lg text-center">🔍 대응 단계</div>
                              <div className="text-slate-300 text-xs text-center mt-1">순서를 조정하거나 에테르 폭주를 토글한 뒤 진행하세요</div>
                            </div>
                          )}
                          {phase==='resolve' && (
                            <div className="bg-slate-800 rounded-lg px-6 py-3 mb-4 mx-auto" style={{width: '400px', maxWidth: '90vw'}}>
                              <div className="text-white font-bold text-lg text-center">▶ 진행 중 ({qIndex}/{queue?.length || 0})</div>
                            </div>
                          )}
                          ...
                          <div className="bg-slate-800/90 rounded-lg px-4 py-2 mt-4" style={{width: '500px', maxWidth: '90vw'}}>
                            <div className="flex items-center gap-2">
                              <div className="text-white font-bold text-xs whitespace-nowrap">📜</div>
                        ...
                        <div className="flex items-center gap-6">
                          <div className="text-right">
                            ...
                            <div className="flex gap-2 flex-wrap justify-end">
                              {enemy.block>0 && <span className="px-3 py-1 rounded-full bg-blue-700 text-white text-sm font-bold">🛡️ {enemy.block}</span>}
                              {enemy.vulnMult>1 && <span className="px-3 py-1 rounded-full bg-red-700 text-white text-sm font-bold">취약 ×{enemy.vulnMult.toFixed(1)}</span>}
                              {enemy.etherOverdriveActive && <span className="px-3 py-1 rounded-full bg-fuchsia-600 text-white text-sm font-bold animate-pulse">⚡폭주</span>}
                            </div>
                            <div className="text-slate-400 text-sm mt-2">적 {enemyIndex+1}/{ENEMIES.length}</div>
                          </div>
                          <div className="character-display">🐉</div>
                        </div>

                        {/* Enemy Ether Bar */}
                        <EtherBar pts={enemy.etherPts} label="에테르" color="fuchsia" />
                      </div>
                </div>
              </div>
              ...
              {phase==='resolve' && queue.length > 0 && (
                <div className="fixed bottom-0 left-0 right-0 submitted-cards-area z-100">
                  <div className="flex items-center justify-between mb-4 max-w-[1600px] mx-auto px-4">
                    <div className="text-white font-black text-2xl">⚔️ 전투 진행 중... ({qIndex}/{queue.length})</div>
              ...
              {phase==='respond' && selected.length > 0 && (
                <div className="fixed bottom-0 left-0 right-0 submitted-cards-area z-100">
                  <div className="flex items-center justify-between mb-4 max-w-[1600px] mx-auto px-4">
                    <div className="text-white font-black text-2xl">📋 제출된 카드</div>
                    ...
                    <div className="flex items-center gap-4">
                      {phase==='select' && (
                        <>
                          ...
                          <div>
                            <div className="text-white font-black text-xl">손패</div>
                      ...
                      {enemy && enemy.hp <= 0 && (
                        <div className="text-emerald-300 font-black text-2xl">🏆 적 처치!</div>
                      )}
                      {player && player.hp <= 0 && (
                        <div className="text-rose-300 font-black text-2xl">💀 패배...</div>
                      )}
                    </div>
