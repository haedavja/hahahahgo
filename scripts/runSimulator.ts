 ...
 *   npx tsx scripts/runSimulator.ts resource [battles]    # 리소스 관리 분석
 *   npx tsx scripts/runSimulator.ts hotspot [battles]     # 전략 핫스팟 분석
 *   npx tsx scripts/runSimulator.ts cumulative [battles]  # 누적 피해 분석
 *   npx tsx scripts/runSimulator.ts recovery [battles]    # 체력 회복 분석
 *   npx tsx scripts/runSimulator.ts priority [battles]    # 우선순위 분석
 *   npx tsx scripts/runSimulator.ts reward [battles]      # 보상 분석
 *   npx tsx scripts/runSimulator.ts turning [battles]     # 전환점 분석
 *   npx tsx scripts/runSimulator.ts bursttiming [battles] # 버스트 타이밍 분석
 *   npx tsx scripts/runSimulator.ts status [battles]      # 상태 이상 분석
 *   npx tsx scripts/runSimulator.ts energy [battles]      # 에너지 효율 분석
 *   npx tsx scripts/runSimulator.ts teamsynergy [battles] # 팀 시너지 분석
 *   npx tsx scripts/runSimulator.ts comeback [battles]    # 역전 가능성 분석
 *   npx tsx scripts/runSimulator.ts loss [battles]        # 손실 분석
 *   npx tsx scripts/runSimulator.ts targeting [battles]   # 표적화 분석
 *   npx tsx scripts/runSimulator.ts interpret [battles]   # 전투 해석 분석
 *   npx tsx scripts/runSimulator.ts endurance [battles]   # 내구력 패턴 분석
...
if (command === 'resource') {
  const battles = parseInt(args[1]) || 30;
  console.log('🎮 리소스 추적 분석 모드\n');
  runResourceTracking(battles);
  process.exit(0);
}

if (command === 'hotspot') {
  const battles = parseInt(args[1]) || 20;
  console.log('🎮 전략 핫스팟 분석 모드\n');
  runStrategyHotspot(battles);
  process.exit(0);
}

if (command === 'cumulative') {
  const battles = parseInt(args[1]) || 30;
  console.log('🎮 누적 피해 분석 모드\n');
  runCumulativeDamage(battles);
  process.exit(0);
}

if (command === 'recovery') {
  const battles = parseInt(args[1]) || 30;
  console.log('🎮 체력 회복 분석 모드\n');
  runHealthRecovery(battles);
  process.exit(0);
}

if (command === 'priority') {
  const battles = parseInt(args[1]) || 30;
  console.log('🎮 우선순위 분석 모드\n');
  runPriorityAnalysis(battles);
  process.exit(0);
}

if (command === 'reward') {
  const battles = parseInt(args[1]) || 30;
  console.log('🎮 보상 분석 모드\n');
  runRewardAnalysis(battles);
  process.exit(0);
}

if (command === 'turning') {
  const battles = parseInt(args[1]) || 30;
  console.log('🎮 전환점 분석 모드\n');
  runTurningPoint(battles);
  process.exit(0);
}

if (command === 'bursttiming') {
  const battles = parseInt(args[1]) || 30;
  console.log('🎮 버스트 타이밍 분석 모드\n');
  runBurstTiming(battles);
  process.exit(0);
}

if (command === 'status') {
  const battles = parseInt(args[1]) || 30;
  console.log('🎮 상태 이상 분석 모드\n');
  runStatusEffectAnalysis(battles);
  process.exit(0);
}

if (command === 'energy') {
  const battles = parseInt(args[1]) || 30;
  console.log('🎮 에너지 효율 분석 모드\n');
  runEnergyEfficiency(battles);
  process.exit(0);
}

if (command === 'teamsynergy') {
  const battles = parseInt(args[1]) || 30;
  console.log('🎮 팀 시너지 분석 모드\n');
  runTeamSynergy(battles);
  process.exit(0);
}

if (command === 'comeback') {
  const battles = parseInt(args[1]) || 30;
  console.log('🎮 역전 가능성 분석 모드\n');
  runComebackPotential(battles);
  process.exit(0);
}

if (command === 'loss') {
  const battles = parseInt(args[1]) || 30;
  console.log('🎮 손실 분석 모드\n');
  runLossAnalysis(battles);
  process.exit(0);
}

if (command === 'targeting') {
  const battles = parseInt(args[1]) || 30;
  console.log('🎮 표적화 분석 모드\n');
  runTargetingAnalysis(battles);
  process.exit(0);
}

if (command === 'interpret') {
  const battles = parseInt(args[1]) || 20;
  console.log('🎮 전투 해석 분석 모드\n');
  runBattleInterpretation(battles);
  process.exit(0);
}

if (command === 'endurance') {
  const battles = parseInt(args[1]) || 30;
  console.log('🎮 내구력 패턴 분석 모드\n');
  runEndurancePatterns(battles);
  process.exit(0);
}

if (command === 'streak') {
  const battles = parseInt(args[1]) || 50;
  console.log('🎮 연승 분석 모드\n');
  runWinStreakAnalysis(battles);
  process.exit(0);
}

if (command === 'deckopt') {
  const battles = parseInt(args[1]) || 30;
  console.log('🎮 덱 최적화 분석 모드\n');
  runDeckOptimization(battles);
  process.exit(0);
}

if (command === 'enemypattern') {
  const battles = parseInt(args[1]) || 30;
  console.log('🎮 적 패턴 예측 분석 모드\n');
  runEnemyPatternPrediction(battles);
  process.exit(0);
}

if (command === 'cardsynergy') {
  const battles = parseInt(args[1]) || 30;
  console.log('🎮 카드 시너지 패턴 분석 모드\n');
  runCardSynergyPatterns(battles);
  process.exit(0);
}

if (command === 'survival') {
  const battles = parseInt(args[1]) || 30;
  console.log('🎮 생존 분석 모드\n');
  runSurvivalAnalysis(battles);
  process.exit(0);
}

if (command === 'attackpattern') {
  const battles = parseInt(args[1]) || 30;
  console.log('🎮 공격 패턴 분석 모드\n');
  runAttackPatternAnalysis(battles);
  process.exit(0);
}

if (command === 'defense') {
  const battles = parseInt(args[1]) || 30;
  console.log('🎮 방어 전략 분석 모드\n');
  runDefenseStrategyAnalysis(battles);
  process.exit(0);
}

if (command === 'combochain') {
  const battles = parseInt(args[1]) || 30;
...
if (command === 'tiercomp') {
  const battles = parseInt(args[1]) || 30;
  console.log('🎮 티어별 비교 분석 모드\n');
  runTierComparisonAnalysis(battles);
  process.exit(0);
}

if (command === 'preseteff') {
  const battles = parseInt(args[1]) || 30;
  console.log('🎮 프리셋 효율 분석 모드\n');
  runPresetEfficiencyAnalysis(battles);
  process.exit(0);
}

if (command === 'weaknessdeep') {
  const battles = parseInt(args[1]) || 20;
  console.log('🎮 적 약점 심화 분석 모드\n');
  runEnemyWeaknessDeepAnalysis(battles);
  process.exit(0);
}

if (command === 'playstyle') {
  const battles = parseInt(args[1]) || 30;
